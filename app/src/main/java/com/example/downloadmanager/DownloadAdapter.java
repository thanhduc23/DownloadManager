    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
        ...
        downloadViewHolder.pause_resume.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                // Get the current position at the time of click.
                int currentPosition = downloadViewHolder.getAdapterPosition();

                // Check if the position is valid.
                if (currentPosition != RecyclerView.NO_POSITION) {
                    // Use currentPosition to access the current item from your dataset.
                    DownloadModel currentDownloadModel = downloadModels.get(currentPosition);

                    if (currentDownloadModel.isIs_paused()) {
                        currentDownloadModel.setIs_paused(false);
                        downloadViewHolder.pause_resume.setText("PAUSE");
                        currentDownloadModel.setStatus("RESUME");
                        downloadViewHolder.file_status.setText("Running");

                        // Attempt to resume the download.
                        if (!resumeDownload(currentDownloadModel)) {
                            Toast.makeText(v.getContext(), "Failed to Resume", Toast.LENGTH_SHORT).show();
                        }
                    } else {
                        currentDownloadModel.setIs_paused(true);
                        downloadViewHolder.pause_resume.setText("RESUME");
                        currentDownloadModel.setStatus("PAUSE");
                        downloadViewHolder.file_status.setText("PAUSE");

                        // Attempt to pause the download.
                        if (!pauseDownload(currentDownloadModel)) {
                            Toast.makeText(v.getContext(), "Failed to Pause", Toast.LENGTH_SHORT).show();
                        }
                    }
                    // Notify the adapter of the item changed.
