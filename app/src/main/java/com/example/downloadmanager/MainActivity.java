...
public class MainActivity extends AppCompatActivity implements ItemClickListener {
    ...
    public void onShareClick(DownloadModel downloadModel) {
        ...
        try{
            ...
            if(Build.VERSION.SDK_INT>=Build.VERSION_CODES.N){
                ...
                Uri path=FileProvider.getUriForFile(MainActivity.this,"com.example.downloadmanage",file);
    ...
    public class DownloadStatusTask extends AsyncTask<String,String,String>{
        ...
        private void downloadFileProcess(String downloadId) {
            ...
            while (downloading) {
                ...
                if (cursor.moveToFirst()) { // Added a check to ensure the cursor is not empty
                    ...
                    if (bytesDownloadedIndex != -1 && totalSizeIndex != -1 && statusIndex != -1) {
                        ...
                        Log.e("DownloadStatusTask", "Required column not found in download manager cursor.");
                    }
                } else {
                    // Handle the case where the cursor is empty (e.g., the downloadId does not match any downloads).
                    Log.e("DownloadStatusTask", "Download ID not found or no downloads in progress.");
                    downloading = false;
                }
                cursor.close();
            }
    ...
    private String getStatusMessage(Cursor cursor) {
        String msg = "-";
        int statusIndex = cursor.getColumnIndex(DownloadManager.COLUMN_STATUS);
        if (statusIndex != -1) { // Check if the status column index is valid
            switch (cursor.getInt(statusIndex)) { // Use the index to retrieve the status
            ...
            Log.e("DownloadStatusTask", "Status column not found in download manager cursor.");
    ...
    BroadcastReceiver onComplete = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (id == -1) {
                // If the EXTRA_DOWNLOAD_ID is not found in the intent or is -1, log an error or handle accordingly
                Log.e("DownloadReceiver", "No download ID found in intent.");
                return;
            }
                ...
                if (cursor != null && cursor.moveToFirst()) {
                    ...
                    if (columnIndex != -1) { // Ensure the column index is valid
                        ...
                        if (downloadedPath != null) {
                            ...
                            // Handle case where the path is null
                            Log.e("DownloadReceiver", "Downloaded path is null.");
                        }
                    } else {
                        // Handle case where the column index is invalid
                        Log.e("DownloadReceiver", "Cannot find COLUMN_LOCAL_URI in cursor.");
                    }
                    cursor.close();
                } else {
                    // Handle case where cursor is empty or null
                    Log.e("DownloadReceiver", "Cursor is empty or null, unable to find download path.");
                    if (cursor != null) {
                        cursor.close();
                    }
                }
            } else {
                // If ChangeItemWithStatus returns false, handle accordingly
                Log.e("DownloadReceiver", "ChangeItemWithStatus returned false.");
            }
        }
    };
